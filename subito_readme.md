...
```powershell
python subito_search-1.py "divano usato"
python subito_search-1.py "tavolo vintage"
python subito_search-1.py "sedia vintage"
python subito_search-1.py "libreria usata"
python subito_search-1.py "comodino antico"
```

### С прокси-сервером (через ключ `-proxy`)

```powershell
python subito_search-1.py "armadio vintage" -proxy 192.168.1.100:8080
```
...
```powershell
python subito_search-1.py "divano vintage" -out F:\_PatinaAi\output
```

### Подробный режим

```powershell
python subito_search-1.py "armadio vintage" -verbose
```
...
```
F:\_PatinaAi\code\
├── subito_search-1.py    # Основной скрипт
├── subito_readme.md      # Эта инструкция
├── prompt0               # Пример промпта
├── prompt1               # Описание задания
└── Descript.ion          # Описание команды запуска
```
